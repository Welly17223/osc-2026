...
extern "C" fn main(hart_id: usize, dtb_address: usize) {
    let mut kernel_base_address: usize = 0x200000; // 0x02000000;
    ...
    for o in &mem_offset[..mem_node_counts] {
        ...
        for i in 0..(len / size_of::<u32>() / 2) as isize {
            let memory_start = ((unsafe { *ptr.wrapping_offset(i * 2) }.swap_bytes() as usize)
                << 32)
            ...
            if !is_set {
                ...
                kernel_base_address += memory_start;
                writeln!(
                    def_uart,
                    "set start memory: {kernel_base_address:#x}, current pc: {pc:#x}"
                )
                .unwrap();
                is_set = true;
            }

            write!(def_uart, "0x{:x}", memory_start).unwrap();
            def_uart.puts(" ");
        }
        def_uart.puts("\n");
    }

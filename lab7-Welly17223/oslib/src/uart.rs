...
impl Uart {
    ...
    pub fn set_interrupt(&mut self, hart_id: usize) {
        ...
        self.tx_queue = Some(RingBuf::default());
...
pub struct UartInode<'a> {
    pub uart: spin::RwLock<&'a mut Uart>,
}

impl<'a> byte_device::ByteDevice for UartInode<'a> {
    fn read(&self, _offset: u64, buf: &mut [u8]) -> Result<usize, VfsError> {
        let _disable_interrupt = SModeInterrupt::new();
        let mut num = 0;
        while num < buf.len() {
            let mut inode = self.uart.write();
            match inode.pop_rx_ch() {
                None => {
                    drop(inode);
                    if current_state() == State::Running {
                        get_rx_thread_queue().push_current();
    ...
    fn write(&self, _offset: u64, buf: &[u8]) -> Result<usize, VfsError> {
        let _disable_interrupt = SModeInterrupt::new();
        let mut inode = self.uart.write();

        unsafe {
            let mut ier = inode.ier().read_volatile();
            ier |= 0b10;
            inode.ier().write_volatile(ier);
        };
        drop(inode);
        ...
        for &s in buf {
            loop {
                let mut inode = self.uart.write();
                let Some(txq) = &mut inode.tx_queue else {

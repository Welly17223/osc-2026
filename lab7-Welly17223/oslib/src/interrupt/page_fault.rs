...
use crate::{
    ...
    virtual_mem::{self, PTE_F, PTE_M, PTE_V, PTE_W, phy_to_virt},
};
...
impl super::InterruptTrait for Interrupt {
    ...
    fn handler(regs: &mut super::pt_regs, interrupt_state: Option<Self::InterruptEnum>) {
        ...
        if let Some(f) = &mut tcb.text_file
            && va < f.len().unwrap() as _
            && !leaf_pte.is_valid()
        {
            let buttom = va & virtual_mem::PAGE_MASK;
            let mut part_text = Box::new([0u8; 4096]);

            let _ = f.seek(crate::file_system::SeekFrom::Start(buttom as _));
            let _ = f.read(part_text.as_mut());
            ...
            virtual_mem::pagewalk(
                root_pgd,
                buttom,
                virtual_mem::virt_to_phy(Box::into_raw(part_text) as _),
                virtual_mem::PROT_USER_TEXT,
            );
            writeln!(serial, "[Translation fault]: {:#x}", regs.stval).unwrap();

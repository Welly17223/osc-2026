...
pub fn dump_tree(fdt: *const u8) {
    let Some(uart_dev) = (unsafe { &mut *(&raw mut uart::SERIAL) }) else {
        return;
    };
    ...
    for (tok, dep) in iter {
        match tok {
            TokenContent::BeginNode { name } => {
                write!(uart_dev, "{:>w$}", "", w = dep * 2).unwrap();
                writeln!(uart_dev, "{name}:\n").unwrap();
            }
            TokenContent::Prop {
                name,
                prop_header: prop,
                ...
                write!(uart_dev, "{:>w$}", "", w = dep * 2).unwrap();
                writeln!(uart_dev, "{name}: {}", prop.len.swap_bytes()).unwrap();
            }
            _ => (),
        }
    }
...
pub fn read_reg(ptr: *const u32) -> (u64, u64) {
    unsafe {
        (
            crate::read_u64_from_ptr_swapbyte(ptr.wrapping_add(0)),
            crate::read_u64_from_ptr_swapbyte(ptr.wrapping_add(2)),
        )
    }
}

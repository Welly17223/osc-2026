...
pub trait InterruptTrait {
    type InterruptEnum: Into<usize> + TryFrom<usize>;
...
impl SModeInterrupt {
    pub fn new() -> Self {
        let is_enable = AtomicBool::new(s_mode_interrupt_status());
...
impl Drop for SModeInterrupt {
    fn drop(&mut self) {
        if self.is_enable.load(core::sync::atomic::Ordering::SeqCst) {
            s_mode_interrupt_enable();
        } else {
            s_mode_interrupt_disable();
        }
...
pub struct SetStatusSUM(AtomicBool);

impl SetStatusSUM {
    pub fn new() -> Self {
        let is_set = riscv::register::sstatus::read().sum();
        unsafe {
            riscv::register::sstatus::set_sum();
        }
        Self(AtomicBool::new(is_set))
    }
}

impl Default for SetStatusSUM {
    fn default() -> Self {
        Self::new()
    }
}

impl Drop for SetStatusSUM {
    fn drop(&mut self) {
        if !self.0.load(core::sync::atomic::Ordering::SeqCst) {
...
impl From<usize> for SupervisorInterrupt0 {
    fn from(value: usize) -> Self {
        match value {
            ...
            9 => SupervisorInterrupt0::EnvironmentCallFromSmode,
            12 => SupervisorInterrupt0::InstructionPageFault,
            13 => SupervisorInterrupt0::LoadPageFault,
...
extern "C" fn do_trap(regs: *mut pt_regs) {
    ...
    if interrupt == 1 {
        match (regs.scause & ((1 << (usize::BITS - 2)) - 1)).try_into() {
            ...
            Ok(SupervisorInterrupt1::Timer) => {
                    ...
                    && entry.0.get_time() < curr_time
                {
                    ...
                    if let Some(new_entry) = entry.next_repeat() {
                        timer_queue.push(core::cmp::Reverse(Box::new(new_entry)));
                    }
                }
            ...
            Ok(SupervisorInterrupt1::External) => {
                ...
                match irq_table.get(&irq) {
                    Some(IRQ::UART) => {
                        let iir_ise = ((uart::get_serial().get_iir() >> 1) & 0b11) as usize;
                        uart::Interrupt::handler(regs, iir_ise.try_into().ok());
                    }
                    // Some(_) => {}
                    None => (),
                }
        ...
        match regs.scause.into() {
            SupervisorInterrupt0::EnvironmentCallFromUmode => {
                ...
                u_mode::Interrupt::handler(regs, regs.a7.try_into().ok());
            }
            SupervisorInterrupt0::InstructionPageFault
            | SupervisorInterrupt0::LoadPageFault
            | SupervisorInterrupt0::StoreAMOPageFault => {
                page_fault::Interrupt::handler(regs, regs.scause.try_into().ok());
            }
            ...
            | SupervisorInterrupt0::LoadAccessFault => {
                serial.puts("\nscause: ");
    ...
    if schedule::current_tcb().sig.sig_mask != 0 {
        let curr_sig = &mut schedule::current_tcb().sig;
        let sig_num: usize = curr_sig.sig_mask.trailing_zeros() as _;
        curr_sig.sig_mask &= !(1 << sig_num);

        if curr_sig.sig_handler_func[sig_num] == 0 {
            thread::do_exit(sig_num as _);
        } else {
            ...
            unsafe {
                (sp as *mut pt_regs).write_volatile(*regs);
            }
            drop(save);

            regs.sepc = curr_sig.sig_handler_func[sig_num];
            regs.sscratch = sp;
            regs.ra = current_tcb().sig.sig_ret_addr;
        }
    }

    if schedule::current_tcb().reschedule {
        current_tcb().reschedule = false;
        schedule::schedule();
    }
}
...
pub fn interrupt_init(dtb_addr: *mut u8, hart_id: usize) -> Result<(), fdt::Error> {
    *IRQ_TABLE.write() = Some(alloc::collections::BTreeMap::new());
    ...
    timer::set_timer_raw(offset_sec(1));
    unsafe {

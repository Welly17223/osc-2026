...
use crate::{schedule, thread, uart};
use core::arch::asm;
    ...
    if schedule::current_tcb().sig.sig_mask != 0 {
        ...
        if curr_sig.sig_handler_func[sig_num] == 0 {
            ...
            let sp: usize = if curr_sig.sig_stack.is_none() {
                let stack = unsafe { Box::<[u8; 0x10000]>::new_uninit().assume_init() };
                let stack_end = stack.as_ptr().wrapping_add(stack.len());
            ...
            regs.sscratch = sp;
            regs.ra = thread::sig_ret as *const () as _;
        }
    }

    if schedule::current_tcb().reschedule {
        let arc = schedule::curr_thread_arc();
        arc.lock().get_mut().reschedule = false;
        schedule::schedule();
    }
}

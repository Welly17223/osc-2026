...
impl DynamicAllocatorHeader32_2048Bytes {
    pub fn new(p: Page, class: Type, prev: usize, next: usize) -> Self {
        ...
        DynamicAllocatorHeader32_2048Bytes {
            ...
            total_entry: (crate::PAGE_SIZE - size_of::<Self>()) / frame_size,
            fragment_page: p,
            used_entry: 0,
...
impl DynamicAllocatorHeader_16Bytes {
    pub fn new(p: Page, prev: usize, next: usize) -> Self {
        DynamicAllocatorHeader_16Bytes {
            ...
            total_entry: (crate::PAGE_SIZE - size_of::<Self>()) / 16,
            fragment_page: p,
            used_entry: [0; 2],
            next_same_size_allocator: next,
            prev_same_size_allocator: prev,
...
impl DynamicAllocator {
    pub fn malloc(&mut self, page_allocator: &mut PageAllocator, size: usize) -> Option<usize> {
        ...
        match self.dynamic_allocator[rounded_exp] {
            Some(head_ptr) if rounded_exp != 0 => {
                ...
                head_meta_data.prev_same_size_allocator = p.addr();
                head_meta_data_prev.next_same_size_allocator = p.addr();

                let meta_data =
                    unsafe { &mut *(p.addr() as *mut DynamicAllocatorHeader32_2048Bytes) };
            ...
            Some(head_ptr) => {
                ...
                let meta_data = unsafe { &mut *(p.addr() as *mut DynamicAllocatorHeader_16Bytes) };
            ...
            None if rounded_exp != 0 => {
                let p = page_allocator.alloc_pages(0, true)?;
                self.dynamic_allocator[rounded_exp] = Some(p.addr());

                let head_addr = p.addr();
                let meta_data =
    ...
    pub fn free(&mut self, ptr: usize) {
        let header = unsafe { &*((ptr & PAGE_MASK) as *mut DynamicAllocatorHeader) };

        match header.class {
            Type::Size16 => {
                let header =
                    unsafe { &mut *((ptr & PAGE_MASK) as *mut DynamicAllocatorHeader_16Bytes) };
                header.free(ptr);
            }
            _ => {
                let header =
                    unsafe { &mut *((ptr & PAGE_MASK) as *mut DynamicAllocatorHeader32_2048Bytes) };
                header.free(ptr);
            }
        }
    }

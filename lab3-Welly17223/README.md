...
第一份需要花很多時間上去的作業，建議多留時間來寫，大約需要 20 小時。雖然目前沒有 memory allocator ，因此整個實體記憶體的所有區域都可以使用，不過要如何<ruby>有效率<rt>低時間複雜度</rt></ruby>的方式管理以及要如何設定 reserved memory 這些概念雖然想起來、看起來不難，但是要如何兼顧 Rust 的<ruby>語法<rt>哲學</rt></ruby>和實做細節確實讓我在寫這一份作業時花的許多時間設計。

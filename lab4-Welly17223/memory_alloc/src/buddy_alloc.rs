...
use crate::{MemoryLayout, MemoryRange, PAGE_MASK, startup_alloc};
...
impl BuddyZone {
    ...
    pub fn find(&self, mem_addr: usize) -> usize {
    ...
    pub fn pages_index(&self, ptr: usize) -> usize {
        ...
        self.allocators[idx].pages_index(ptr)
    }

    pub fn pages_state(&self, ptr: usize) -> PageState {
        let idx = self.find(ptr);
        self.allocators[idx].pages_state(ptr)
    }
}

impl fmt::Display for BuddyZone {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "avaliable_memory: {:#x?}", self.memory_layout.avaliable_memory)?;
        write!(f, "reserved_memory: {:#x?}", self.memory_layout.reserved_memory)?;
...
impl PageAllocator {
    ...
    pub fn new(
        startup_alloc: &mut startup_alloc::StartupAllocator,
        reserved_memory: &[crate::MemoryRange],
        range: MemoryRange,
    ) -> Result<PageAllocator, crate::Error> {
        ...
        'find_free_list: while curr_pages_init_count < page_count {
            ...
            log::trace!(
                ...
                curr_order, curr_pages_init_count, next_idx
            );
...
impl FreePageList {
    ...
    pub fn pop_head(&mut self) {
        ...
        let head_node = h as *const FreePageNode;
        self.header = unsafe { *head_node }.next;
        ...
        if let Some(n) = self.header {
            unsafe { *(n as *mut FreePageNode) }.prev = None;
        }
    }

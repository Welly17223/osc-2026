...
use crate::{buddy_alloc::Page, dynamic_alloc::DynamicAllocator};
...
unsafe impl alloc::GlobalAlloc for GlobalAllocator {
    unsafe fn alloc(&self, layout: alloc::Layout) -> *mut u8 {
        let _disable_interrupt = oslib::interrupt::SModeInterrupt::new();
        let res = if let Some(allocator) = self.allocator.lock().as_mut() {
...
impl MemoryAllocator {
    ...
    pub fn free(&mut self, ptr: usize) {
        ...
        let state = self.buddy_allocators.pages_state(ptr & PAGE_MASK);
        let index = self.buddy_allocators.pages_index(ptr & PAGE_MASK);
        let order = match state {
            ...
            PageState::Occupied(o) => {
                let page_idx = self.buddy_allocators.pages_index(ptr);
                let ptr = align(ptr, 4096);
...
pub fn get_memory_layout(
    ...
    let mut reserved_memory_node = [0usize; 16];
...
pub fn init_memory_allocator(
    ...
    // let range = memory_layout.avaliable_memory[0];
